        ...
        <BackgroundRank>
          ...
          <StyledTable border="1px" cellPadding="5px" cellSpacing="0">
            <thead style={{ backgroundColor: "#C0C0C0" }}>
              ...
              <th>Awards</th>
              <th>
                <span>{score ? "Monthly" : "Yearly"}</span> score
              </th>
              <th>Website</th>
              <th>
                <details>
                  <summary>Category</summary>
                  <Option onClick={() => setCategory('Sem filtro')}>Remover filtro</Option>
                  <Option onClick={() => setCategory('Supermercado/Hipermercado')}>Supermercado/Hipermercado </Option>
                  <Option onClick={() => setCategory('Restaurante/Bar')}>Restaurante/Bar</Option>
                  <Option onClick={() => setCategory('Indústria')}>Indústria</Option>
                  <Option onClick={() => setCategory('Mercearia')}>Mercearia</Option>
                  <Option onClick={() => setCategory('Drogaria')}>Drogaria</Option>
                  <Option onClick={() => setCategory('Shopping')}>Shopping</Option>
                  <Option onClick={() => setCategory('Coleta')}>Coleta</Option>
                  <Option onClick={() => setCategory('Padaria')}>Padaria</Option>
                  <Option onClick={() => setCategory('Varejista')}>Varejista</Option>
                  <Option onClick={() => setCategory('Hotel/Motel')}>Hotel/Motel</Option>
                  <Option onClick={() => setCategory('Condominio')}>Condominio</Option>
                </details>
              </th>
              <th>
                <details>
                  <summary>Filter by business size</summary>
                  <Option onClick={() => setScore("mensal")}>
                    Remover filtro
                  </Option>
                  <Option onClick={() => setScore("microempresa")}>
                    Micro
                  </Option>
                  <Option onClick={() => setScore("pequena")}>Pequena</Option>
                  <Option onClick={() => setScore("media")}>Média</Option>
                  <Option onClick={() => setScore("grande")}>Grande</Option>
                </details>
              </th>
            </thead>
            <tbody>
              ...
              {score !== ("anual" || "mensal") && category === "Sem filtro"
                ? business
                    .sort(orderByScoreYear)
                    .filter((item) => item.businessSize === score)
                    .map((item, index) => (
                      <Tr key={index}>
                        <Td>{index + 1}ª</Td>
                        <Td
                ...
                : business
                    .sort(orderByScoreYear)
                    .filter((item) => item.businessSize === score && item.business === category)
                    .map((item, index) => (
                      <Tr key={index}>
                        <Td>{index + 1}ª</Td>
                        <Td
